...
* Open a log file
cap log close
log using ./logs/01_longCovidSymp_split_by_stp_omicron.log, replace t
...
foreach i of numlist 5/9 {
	preserve
		...
		capture noisily export delimited using "./output/input_covid_communitycases_stp`i'_omicron.csv", replace
		count
...
foreach i of numlist 5/9  {
	preserve
		capture noisily keep if stp=="E5400000`i'"
		capture noisily export delimited using "./output/input_controls_contemporary_stp`i'_omicron.csv", replace
		count
		capture noisily safetab stp
	restore
}
