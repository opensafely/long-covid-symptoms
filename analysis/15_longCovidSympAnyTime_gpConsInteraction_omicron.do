...
capture log close
log using ./logs/15_longCovidSympAnyTime_gpConsInteractioncontemporary_omicron.log, replace t
	...
	*loop through categories of gp count in previous year (0-2)
	forvalues i=0/2 {
		...
		*forvalues n=0/1 {
			...
			capture noisily lincom 1.expStatus + 1.expStatus#`i'.gpCountPrevYearCat, or
...
file open tablecontents using ./output/Figure4_gpConsInteraction_contemporary_omicron.txt, t w replace
file write tablecontents "Fig 4: Analysis of symptoms (at any time) including interaction with age for contemporary_omicron comparator population (full adjusted)" _n _n
...
prog define outputORsforOutcomeSENS
	...
	*loop through categories of gp count in previous year (0-2)
	forvalues i=0/2 {
		...
		*loop through categories of exposure (0-1)
			capture noisily estimates restore fullAdj
			capture noisily lincom 1.expStatus + 1.expStatus#`i'.gpCountPrevYearSENS, or
			capture noisily local hr_fulladj = r(estimate)
			capture noisily local lb_fulladj = r(lb)
			capture noisily local ub_fulladj = r(ub)
			file write tablecontents  _tab %4.2f (`hr_fulladj')  " (" %4.2f (`lb_fulladj') "-" %4.2f (`ub_fulladj') ")"  _n
...
*JUST for delirium
keep if age>=67
cap noisily outputORsforOutcomeSENS, outcome(tEver_symp_delirium)
file write tablecontents _n
