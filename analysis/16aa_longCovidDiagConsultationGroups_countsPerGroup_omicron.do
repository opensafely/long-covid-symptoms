...
*
*ALL TABULATIONS FOR CONS INTERACTION IE SYMPTOMS, DIAGNOSES AND MEDICINES
*
...
capture log close
log using ./logs/16aa_longCovidDiagConsultationGroups_countsPerGroupcontemporary_omicron.log, replace t
...
local totExp=r(N)
file open tablecontents using ./output/Figure5_gpConsInteractionCounts_contemporary_omicron.txt, t w replace
file write tablecontents "Fig 5: Supporting figures. Number with events by exposure satus at any time BY PREVIOUS CONSULTATIONS during follow up (contemporary_omicron comparator)" _n _n
...
**********************************REPEAT FOR SENSITIVITY ANALYSIS PREV CONS************************
prog define outputCountsforOutcomeSENS
	syntax, outcome(string)
	...
	forvalues i=0/2{
		*get overall denominator for the specific strata of previous consultation			
		count if gpCountPrevYearSENS==`i'
		local denom=r(N)
		
		*get number of people with specific outcome (events column)
		cou if gpCountPrevYearSENS==`i' & `outcome'==1
		local events=round(r(N),5)
		*calculate proportion of people with events within the specific previous consultation strata
		local percWEvent=100*(`events'/`denom')
		*get exposed vs unexposed denominators			
		count if gpCountPrevYearSENS==`i' & expStatus==0
		local unexpDenom=round(r(N),5)
		count if gpCountPrevYearSENS==`i' & expStatus==1
		local expDenom=round(r(N),5)
		*get number of people (and %) with events by exposure status 
		cou if gpCountPrevYearSENS==`i' & `outcome'==1 & expStatus==0
		local unexpEvents=round(r(N),5)
		local unexpPercWEvent=100*(`unexpEvents'/`unexpDenom')
		cou if gpCountPrevYearSENS==`i' & `outcome'==1 & expStatus==1
		local expEvents=round(r(N),5)
		local expPercWEvent=100*(`expEvents'/`expDenom')	
		...
		*get prevconsname
		local catLab: label (gpCountPrevYearSENS) `i'
		display "Category label: `catLab'"
...
foreach outcome in $symp {
	cap noisily outputCountsforOutcomeSENS, outcome(tEver_`outcome')
	file write tablecontents _n
}

*loop through DIAGNOSES
file write tablecontents "DIAGNOSES - PREV GPCONS SENSITVITY ANALYSIS" _n _n
foreach outcome in $diag {
	cap noisily outputCountsforOutcomeSENS, outcome(tEver_`outcome')
	file write tablecontents _n
}
...
*loop through PRESCRIPTIONS
file write tablecontents "PRESCRIPTIONS - PREV GPCONS SENSITVITY ANALYSIS" _n _n
foreach outcome in $medicines {
	cap noisily outputCountsforOutcomeSENS, outcome(tEver_`outcome')
	file write tablecontents _n
}
