...
capture log close
log using ./logs/15aa_longCovidSympConsultationGroups_countsPerGroup`dataset'.log, replace t
	...
	*get overall denominator			
	...
	forvalues i=0/2{
		*get number of people with specific outcome (events column)
		cou if `outcome' == 1 & gpCountPrevYearCat == `i'
		...
		*get number of people (and %) with events by exposure status 
		cou if `outcome' == 1 & gpCountPrevYearCat == `i' & expStatus==0
		local unexpEvents=round(r(N),5)
		local unexpPercWEvent=100*(`unexpEvents'/`unexpDenom')
		cou if `outcome' == 1 & gpCountPrevYearCat == `i' & expStatus==1
		...
		*get variable name
		...
		display "`varLab'"
		local catLab`i': label(gpCountPrevYearCat) `i'
		
		*write each row
		*crude 
		file write tablecontents  ("`varLab'") _tab ("`catLab`i''")  _tab (`denom') _tab (`events')  _tab %3.1f (`percWEvent') ("%") _tab (`unexpDenom') _tab (`unexpEvents')  _tab %3.1f (`unexpPercWEvent') ("%") _tab (`expDenom') _tab (`expEvents')  _tab %3.1f (`expPercWEvent') ("%")  _n
	}
end
...
count if expStatus==1
local totExp=r(N)
file open tablecontents using ./output/Figure4_gpConsInteractionCounts_`dataset'.txt, t w replace
file write tablecontents "Fig 1: Panel A supporting figures. Number with events by exposure satus AT ANY TIME during follow up, by consultation group (`dataset' comparator)" _n _n
file write tablecontents ("Unexposed to COVID N=`totUnexp'") _tab ("Exposed to COVID N=`totExp'") _n _n
file write tablecontents _tab ("Cons in prev yr") _tab ("Total N") _tab ("n with event") _tab ("% of total with events") _tab ("Unexposed N") _tab ("n unexposed with event") _tab ("% of unexposed with events") _tab ("Exposed to COVID N") _tab ("n exposed to COVID with event") _tab ("% of exposed to COVID with events") _n
