...
foreach i of numlist 5/10 12/17 20/27 29 33 35/37 40/44 49 {
	capture noisily import delimited ./output/matched_cases_historical_stp`i'.csv, clear
	*FOR TESTING ON DUMMY DATA
	*import delimited ./output/input_covid_communitycases_stp`i'.csv, clear
	capture noisily tempfile matched_cases_historical_stp`i'
	capture noisily save `matched_cases_historical_stp`i'', replace
}

*(2)=========Append separate cases files==========
use `matched_cases_historical_stp5', clear
foreach i of numlist 6/10 12/17 20/27 29 33 35/37 40/44 49 {
	capture noisily append using `matched_cases_historical_stp`i'', force
}
...
capture noisily export delimited using "./output/input_covid_matched_cases_historical_allSTPs.csv", replace
