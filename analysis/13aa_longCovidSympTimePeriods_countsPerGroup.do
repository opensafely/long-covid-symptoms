...
capture log close
log using ./logs/13aa_longCovidSympTimePeriods_countsPerGroup`dataset'.log, replace t
...
count if expStatus==1
local totExp=r(N)
file open tablecontents using ./output/figure2PanelA_countsPerGroup_`dataset'.txt, t w replace
...
cap noisily outputCountsforOutcome, outcome(symp_rashes)
file write tablecontents _n
