...
capture log close
log using ./logs/13aa_longCovidSympTimePeriods_countsPerGroupcontemporary_omicron.log, replace t
...
count if expStatus==1
local totExp=r(N)
file open tablecontents using ./output/figure2PanelA_countsPerGroup_contemporary_omicron.txt, t w replace
file write tablecontents "Fig 1: Panel A supporting figures. Number with events by exposure satus AT ANY TIME during follow up (contemporary_omicron comparator)" _n _n
...
cap noisily outputCountsforOutcome anySymptomsEver contemporary_omicron
file write tablecontents _n

*loop through each outcome
foreach outcome in $symp {
	cap noisily outputCountsforOutcome `outcome' contemporary_omicron
	file write tablecontents _n
}

*JUST FOR DELIRIUM
keep if age>=67
cap noisily outputCountsforOutcome symp_delirium contemporary_omicron
file write tablecontents _n
