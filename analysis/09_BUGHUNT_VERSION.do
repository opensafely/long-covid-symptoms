...
* Open a log file
cap log close
log using ./logs/09_longCovidSymp_cr_contemporary_analysis_dataset.log, replace t
...
keep patient_id age case covid_hosp covid_tpp_prob covid_tpp_probw2 death_date dereg_date first_known_covid19 first_pos_test first_pos_testw2 had_covid_hosp has_died has_follow_up imd match_counts pos_covid_test_ever sex stp
...
/*
*DON'T NEED has_follow_up, has_died from the original file - these are all created new based on case_index_date in the new file
*/
capture noisily import delimited ./output/input_covid_matched_matches_contemporary_allSTPs.csv, clear
keep patient_id age case covid_hosp covid_tpp_prob covid_tpp_probw2 first_known_covid19 first_pos_test first_pos_testw2 imd pos_covid_test_ever set_id sex stp death_date dereg_date
...
save `case_medicines'
capture noisily import delimited ./output/input_medicines_controls_contemporary.csv, clear
count
drop case_index_date
...
***NEW CODE DUE TO BUGHUNT - first_known_covid needs to be date format before appending as it is already date format in the cases cohort
capture noisily confirm string variable first_known_covid
capture noisily rename first_known_covid first_known_covid_dstr
capture noisily gen first_known_covid = date(first_known_covid_dstr, "YMD")
capture noisily drop first_known_covid_dstr
capture noisily format first_known_covid %td 
...
/*ORIGINAL CODE BEFORE BUGHUNT: append using `cases_with_vars_and_match_info', force*/
*Using force here was messing up dereg_date
...
preserve
	...
	*save file for descriptive analysis
	save output/longCovidSymp_UnmatchedCases_contemporary_analysis_dataset.dta, replace
...
*temporary fix for dereg_date issue


*(f) Recode all dates from the strings 
*sort out dereg_date bug (temporary fix)
...
*found the bug - this command drops all of the control dereg_dates!

*bughunt*
sum dereg_date if case==0
