...
capture log close
log using ./logs/16_longCovidSympTimePeriods_gpConsInteraction`dataset'.log, replace t
	...
	*loop through each time period
	forvalues n=1/3 {
		*Fully adjusted but including an interaction with previous consultations
		capture noisily clogit t`n'_`outcome' i.expStatus##i.gpCountPrevYearCat i.imd i.ethnicity i.rural_urban i.numPreExistingComorbs, strata(set_id) or
		capture noisily estimates store fullAdj
		*get and diplay variable name
		local varLab: variable label t`n'_`outcome'
		display "`varLab'"
...
file write tablecontents "Fig 5: Analysis of symptoms (over three time periods) including interaction with age for `dataset' comparator population (full adjusted)" _n _n
