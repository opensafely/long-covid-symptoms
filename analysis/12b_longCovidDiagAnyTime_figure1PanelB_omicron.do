...
capture log close
log using ./logs/12b_longCovidDiagAnyTime_figure1PanelBcontemporary_omicron.log, replace t
...
prog define outputORsforOutcome
	...
	*write each row
	*crude 
	file write tablecontents  ("`varLab'") _tab ("vs contemporary_omicron") _tab ("Crude") _tab %4.2f (`hr_crude')  " (" %4.2f (`lb_crude') "-" %4.2f (`ub_crude') ")" _tab (`events') _tab %3.1f (`percWEvent') ("%")  _n
...
file open tablecontents using ./output/figure1PanelB_longCovidDiagAnyTime_contemporary_omicron.txt, t w replace
